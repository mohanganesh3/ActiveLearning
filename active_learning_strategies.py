...
class AdvancedLeader:
    ...
    **Why it NOW works on both:**
    ...
    def __init__(self, N, budget, total_rounds=9):
        self.N = N
        self.budget = budget
        self.total_rounds = total_rounds  # V3: For late-round awareness
        self.num_classes = None  # Auto-detected from predictions
        self.prev_thresholds = None  # V3: For threshold validation
        self.current_round = 0  # V3: Track round number
    
    def select_batch(self, model, unlabeled_data, round_num=None):
        model.eval()
        device = next(model.parameters()).device
        
        # V3: Update round tracking
        if round_num is not None:
            self.current_round = round_num
        
        # Extract features, uncertainties, AND predictions
        print("   [V3] Extracting features + uncertainties + predictions...")
        ...
        print(f"   [V3] Detected {self.num_classes} classes in unlabeled data")
        ...
        # V3: Score and select with IMPROVED CLASS DIVERSITY
        if len(candidate_leaders) > self.budget:
            selected = self._score_and_select_v3(
        ...
        print(f"   [V3] Final selection: {len(selected)} samples")
        return selected[:self.budget]
    ...
    def _compute_multi_scale_thresholds(self, features, round_num=None, prev_thresholds=None):
        """
        V3: UNIVERSAL Adaptive Multi-scale Thresholds with Late-Round Awareness
        ...
        V3 Improvement: In late rounds (7-9), increase selectivity to avoid picking
        noise from the depleted unlabeled pool.
        ...
        # V3: Late-round selectivity adjustment (rounds 7-9 out of 9)
        if round_num is not None and self.total_rounds is not None:
            progress = (round_num + 1) / self.total_rounds  # +1 because rounds are 0-indexed
            if progress >= 0.7:  # Last 30% of rounds
                # Increase percentiles to be MORE selective
                late_factor = 1.0 + 0.15 * (progress - 0.7) / 0.3  # Max 1.15x at final round
                base_percentiles = [p * late_factor for p in base_percentiles]
                print(f"   [V3 Late Round {round_num+1}/{self.total_rounds}] Selectivity boost: {late_factor:.3f}x")
        
        # Compute thresholds from adapted percentiles
        p_fine = float(np.percentile(distances, min(base_percentiles[0], 95)))  # Cap at 95th percentile
        p_med = float(np.percentile(distances, min(base_percentiles[1], 97)))
        p_coarse = float(np.percentile(distances, min(base_percentiles[2], 99)))
        ...
        # V3: Sanity check against previous thresholds (validation, not enforcement)
        if prev_thresholds is not None:
            for i, (new_t, prev_t) in enumerate(zip(new_thresholds, prev_thresholds)):
                ...
                if ratio > 2.0 or ratio < 0.5:
                    print(f"   ⚠️ [V3] Threshold[{i}] changed by {ratio:.2f}x: {prev_t:.3f} → {new_t:.3f}")
        
        # Safety: ensure minimum thresholds
        new_thresholds[0] = max(new_thresholds[0], 0.5)
        new_thresholds[1] = max(new_thresholds[1], 1.0)
        new_thresholds[2] = max(new_thresholds[2], 2.0)
    ...
    def _score_and_select_v3(self, candidates, features, uncertainties, densities, predictions):
        """
        V3: Improved scoring with gentle class-diversity bonus
        
        Score candidates by:
        - Uncertainty (35%): prefer uncertain samples
        - Density (25%): prefer low density regions  
        - Diversity (25%): prefer distant from already selected
        - Class balance (15%): gentle bonus for under-represented classes
        
        V3 Improvement: Track selected leaders' classes and give gentle bonus
        to underrepresented classes (without forcing).
        """
        scores = []
        selected = []
        ...
        for idx in candidates:
            ...
            # V3: Gentle class-diversity bonus
            pred_class = predictions[idx]
            class_freq = selected_class_counts.get(pred_class, 0)
            # Gentle bonus: 1.0 for new class, 0.91 for freq=1, 0.83 for freq=2, etc.
            # Max penalty is ~0.5 for very frequent classes
        ...
        # Select top candidates incrementally (so class counts update)
        scores.sort(key=lambda x: x[1], reverse=True)
        for idx, score in scores[:self.budget]:
            selected.append(idx)
            pred_class = predictions[idx]
            selected_class_counts[pred_class] = selected_class_counts.get(pred_class, 0) + 1
